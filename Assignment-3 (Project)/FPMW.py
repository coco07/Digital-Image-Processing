...
# multi-scale median filtering detection.

# USAGE: FPMW(image_name, class_).
# does our proposed Four Perturbations Multiple Windows blind forensic technique on image_name of class 
# class (-1 for pristine or 1 for median filtered image).
...
def FPMW(image_name, class_):
    ...
    # First, the image is progressively blurred four times with a 3x3 window
    ...
    blurred_image_4_3x3 = cv2.medianBlur(blurred_image_3_3x3, 3)
    ...
    # Now, the image is progressively blurred four times with a 5x5 window
    # at each blurry, the blurred image is compared to the input image by means of image quality metrics.
    # we use eight image quality metrics in the comparison
    ...
    blurred_image_4_5x5 = cv2.medianBlur(blurred_image_3_5x5, 5)
    ...
    # Now, the image is progressively blurred four times with a 7x7 window
    # at each blurry, the blurred image is compared to the input image by means of image quality metrics.
    # we use eight image quality metrics in the comparison
    ...
    blurred_image_4_7x7 = cv2.medianBlur(blurred_image_3_7x7, 7)
    ...
    blurred_image_4_9x9 = cv2.medianBlur(blurred_image_3_9x9, 9)
    temp_vec = calculate_IQMs(image, blurred_image_4_9x9)
    feat_vec[121] = temp_vec[0]
...
if __name__ == '__main__':
    ...
    np.savetxt("FPMW_feat_vec.txt", np_array, fmt="%0.2f")
    print("feature vector saved in FPMW_feat_vec.txt file...")
